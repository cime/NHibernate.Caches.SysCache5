...
namespace NHibernate.Caches.SysCache5
{
	/// <summary>
	...
	public class ConfigConnectionStringProvider : IConnectionStringProvider
	{
		...
		public ConfigConnectionStringProvider()
		{
			...
			{
				throw new ConfigurationErrorsException("There are no configured connection strings.");
			}
		}
		...
		public string GetConnectionString(string name)
		{
			...
			if (connectionSettings == null)
			{
				throw new ConfigurationErrorsException(
					String.Format("Named connection string, '{0}', could not be found by configured connection string provider.", name));
			}
